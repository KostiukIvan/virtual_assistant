...
class AdvancedSpeechPauseDetector:
    ...
    def __init__(self,
        ...
        if sample_rate not in [8000, 16000, 32000, 48000]:
            raise ValueError("Unsupported sample rate. Must be 8k, 16k, 32k, or 48k.")
        if frame_duration_ms not in [10, 20, 30]:
            raise ValueError("Unsupported frame duration. Must be 10, 20, or 30 ms.")
        ...
        print("AdvancedSpeechPauseDetector initialized with:")
        print(f" - VAD Aggressiveness: {vad_level}")
        print(f" - Short Pause: > {short_pause_ms} ms ({self._min_short_pause_frames:.1f} frames)")
        print(f" - Long Pause: > {long_pause_ms} ms ({self._min_long_pause_frames:.1f} frames)\n")
...
def main():
    ...
    print("Starting audio stream... Speak and pause to see detections.")
        ...
        with sd.InputStream(
            ...
            while True:
                ...
                if status not in ["SPEECH", "SILENCE"]:
                    duration_ms = detector.consecutive_silent_frames * FRAME_DURATION_MS
                    print(f"EVENT: {status} (after {duration_ms} ms of silence)")
